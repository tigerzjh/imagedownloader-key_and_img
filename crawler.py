...
def google_gen_query_url(keywords, face_only=False, safe_mode=False, image_type=None, color=None):
    ...
    filter_url = "&tbs=" #"https://www.google.com/search?tbm=isch&hl=en"+ "&q=" + key + "&tbs="
    ...
    query_url += filter_url  #'https://www.google.com/search?tbm=isch&hl=en&q=D%3A/PycharmProject/Image-Downloader/image_downloader.py&safe=off'
    return query_url
...
def google_image_url_from_webpage(driver, max_number, quiet=False):
    ...
    while True:
        try:
            ...
            my_print("Find {} images.".format(len(thumb_elements)), quiet) #len(thumb_elements) =20
...
def crawl_image_urls(img_url, keywords,engine="Google", max_number=10000,
    ...
    if engine != "Baidu":
        browser = str.lower(browser) #'chrome_headless'
        if "chrome" in browser:
            chrome_path = shutil.which("chromedriver") #'D:\\Program Files (x86)\\chromedriver.EXE'
    ...
    #                                        proxy=proxy, proxy_type=proxy_type)#image_urls长度为20的list
    if engine != "Baidu":
        driver.close() #关闭

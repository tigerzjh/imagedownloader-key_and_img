...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.pushButton_load_file.clicked.connect(
            ...
            self, "Choose input images file directory", "./")))
    ...
    def gen_config_from_ui(self):
        ...
        config = utils.AppConfig() #self.keywords = ""  self.output_dir = "./output"
        ...
        """ input directory """
        config.input_dir = self.lineEdit_path2file.text() #mine_need_to_delete
        """ Switches """
        ...
        """ Keywords List """
        if self.checkBox_from_file.isChecked(): #load file button
            image_names = os.listdir(config.input_dir)
            keywords_list = image_names
        else:
            str_keywords = self.lineEdit_keywords.text()
            keywords_list = utils.gen_keywords_list_from_str(str_keywords, ",") #split by comma
    ...
    def start_download(self):
        ...
        for index in range(num_keywords):
            ...
            if keywords == "":
                continue
            if self.checkBox_from_file.isChecked(): #image-file-mode
                config.out_file_name = keywords.split('.')[0] #control outputpath / outputpath can not have .jpg
                config.inputimage = keywords  # need_delete #control inputpath
            else:
                config.keywords = config.out_file_name = keywords

            str_paras = config.to_command_paras() #utils.py line 33 ,generate command

...
class My_Google:
    ...
    def __init__(self, **request_kwargs):
        ...
        self.requests_kwargs = request_kwargs #dict0
    ...
    def _slice(res): 
        ...
        resp = soup.find_all(class_ = 'ekf0x hSQtef') #edit here
        a_bf = BeautifulSoup(str(resp[0]))
        img_url = a_bf.find_all('a')[0].get('href')
        return ('https://www.google.com' + img_url + "&safe=off")
    ...
    def search(self, url):
        ...
        try:
            ...
            if url[:4] == 'http':
                ...
                response = requests.post(f"{self.GOOGLEURL}/upload", files=multipart, headers=self.header, **self.requests_kwargs)
            if response.status_code == 200: #response normal
